...
from ExerciseAiTrainer import (
    Exercise, relevant_landmarks_indices,
...
class WebRTCExerciseProcessor(VideoProcessorBase):
    ...
    def __init__(self):
        ...
        # Map exercise names to counting functions
        self._count_funcs = {
    ...
    def get_state(self):
        """Thread-safe read of current state for UI display."""
        with self._lock:
            return {
                ...
                "calories": estimate_calories(self.exercise_name, self.counter),
...
class WebRTCAutoClassifyProcessor(VideoProcessorBase):
    ...
    def __init__(self):
        ...
        # Counting state for all exercises
    ...
    def _load_train_info(self):
        try:
            ...
            if info_path.exists():
                ...
                classes = self.exercise_instance.exercise_classes
    ...
    def get_state(self):
        with self._lock:
            ...
            counter_map = {'push-up': 'push_up', 'squat': 'squat', 'barbell biceps curl': 'bicep_curl', 'shoulder press': 'shoulder_press'}
            ...
            for key, count in self.counters.items():
                if count > 0:
                    ...
                    cal = estimate_calories(ex_name, count)
                    total_cal += cal
                    breakdown[exercise_name_map.get(key, key)] = {"reps": count, "calories": round(cal, 1)}
            return {
                "prediction": canonical_to_display_name(self.canonical_prediction) if self.canonical_prediction else "Detecting...",
    ...
    def recv(self, frame):
        ...
        if not self.model_ready:
            h, w = img.shape[:2]
            draw_styled_text(img, "Model not loaded - check models/ directory", (10, h // 2), font_scale=0.7, font_color=(0, 0, 255))
            return av.VideoFrame.from_ndarray(img, format="bgr24")
        ...
        # Pose detection for counting/drawing
        img = self.detector.find_person(img, draw=True)
        ...
        if len(landmark_list) > 0 and canon in self.known_exercises:
            with self._lock:
                if canon == 'push-up':
                    self.stages['push_up'], self.counters['push_up'] = count_repetition_push_up(
                        self.detector, img, landmark_list, self.stages['push_up'], self.counters['push_up'], ex)
                elif canon == 'squat':
                    self.stages['squat'], self.counters['squat'] = count_repetition_squat(
                        self.detector, img, landmark_list, self.stages['squat'], self.counters['squat'], ex)
                elif canon == 'barbell biceps curl':
                    self.stages['right_bicep_curl'], self.stages['left_bicep_curl'], self.counters['bicep_curl'] = count_repetition_bicep_curl(
                        self.detector, img, landmark_list, self.stages['right_bicep_curl'], self.stages['left_bicep_curl'], self.counters['bicep_curl'], ex)
                elif canon == 'shoulder press':
                    self.stages['shoulder_press'], self.counters['shoulder_press'] = count_repetition_shoulder_press(
                        self.detector, img, landmark_list, self.stages['shoulder_press'], self.counters['shoulder_press'], ex)

            # Injury/form feedback
            now = time.time()
        ...
        # Left sidebar for counters
        num_ex = len(counters)
        v_spacing = h // (num_ex + 1)
